...
if(isset($_POST['submit'])){
    ...
    $query = "UPDATE datapendaftartsana SET firstName=?, lastName=?, gender=?, email=?, number=?, harga=?, bukti=?, transaksi_status=? WHERE id=?";
    $stmt = $koneksi->prepare($query);
    $stmt->bind_param('ssssssssi', $firstName, $lastName, $gender, $email, $number, $harga, $bukti, $transaksi_status, $id);
    $stmt->execute();
    ...
    $query = "SELECT * FROM datapendaftartsana WHERE id=?";
    $stmt = $koneksi->prepare($query);
...
<form action="editT.php" method="post">
    ...
    <input type="text" id="number" name="number" value="<?php echo $data['number']; ?>"><br>
